...
This vault is a literature and knowledge database for optics/photonics research. Its
purpose is not merely to store paper summaries, but to surface reusable mechanisms,
cross-field analogies, and research directions that can inspire new experiments and
concepts. Maintain a high-level understanding of all papers in the directory, with
special attention to interconnections across materials, device architectures, nonlinear
processes, quantum optics, integrated photonics, computation, sensing, and communication.

When ingesting papers, prefer synthesis over subfield isolation: each source page should
identify what design pattern, physical mechanism, fabrication route, or research question
the paper contributes to the broader map.
...
3. **Schema** (this file, `AGENTS.md`) — conventions, workflows, and templates.
